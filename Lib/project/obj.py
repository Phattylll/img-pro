...
def load_model():
    ...
    if loaded_model is None:
        loaded_model = tf.keras.models.load_model(r'D:\3.1\4.1\ImgPro\Lib\food_classtest3')
    return loaded_model
...
def predict_class(img_path):
    try:
        ...
        if confidence >= 50:
            ...
            ranked_class_items = [{'item': entry['item'], 'confidence': f"{entry['confidence']:.2f}%"} for entry in class_items_with_confidence]
